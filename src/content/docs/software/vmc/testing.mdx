...
On a clean build, CMake fetches Catch2 3.8.1 from GitHub. The first sandboxed
attempt in the documentation workspace failed because DNS/network access was
blocked. The actual terminal error was:
...
CMake Error at catch2-subbuild/catch2-populate-prefix/tmp/catch2-populate-gitclone.cmake:39 (message):
  Failed to clone repository: 'https://github.com/catchorg/Catch2.git'
```

After network access was allowed, the same command built the tests and
completed successfully. The final portion of that real run was:
...
These timings are observations from the documentation workspace, not UWHPC
