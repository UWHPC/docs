...
These numerical values are one observed CPU run with the checked-in
configuration. They are not an authoritative benchmark or a promised result on
UWHPC hardware.
...
precision, or final aggregate record. `render.py` assumes the FP64 layout shown
above.

:::caution[FP32 compatibility]
The writer serializes the build-dependent `real_t`, while `render.py` always
reads float64 values. Do not assume output from an FP32 or fast-math build is
compatible with the bundled renderer without updating or validating the file
format.
:::
